...
const API_ENDPOINT = 'https://eoud8rc95130s2b.m.pipedream.net/'
...
// Coerce any value into a clean array of strings.
function toStringArray(value: unknown): string[] {
  if (Array.isArray(value)) {
    return value.map((v) => (typeof v === 'string' ? v : String(v))).filter((v) => v.trim())
  }
  if (typeof value === 'string' && value.trim()) {
    // Allow newline / comma separated strings for tags & lists.
    return value
      .split(/\r?\n|,/)
      .map((v) => v.trim())
      .filter(Boolean)
...
// Normalize whatever the Pipedream endpoint returns into our four-tab shape.
function normalizeResult(data: unknown): AiData {
  if (data && typeof data === 'object') {
    const obj = data as Record<string, unknown>
    // Some webhook wrappers nest the payload under `body` or `result`.
    const inner =
...
export function OmnicoreApp() {
  ...
  // Data returned from the Pipedream API
  ...
  // Bypass Key Verify Function (Owner System)
  const verifyOwnerKey = () => {
    if (secretKeyInput === 'BHAI_OWNER_2026') {
      setIsOwner(true)
      setCredits(99999)
      alert('Welcome Boss! Owner Mode Activated. Unlimited Credits Granted.')
    } else {
      alert('Galat Key Hai Bhai! Sirf Owner allowed hai.')
    }
  }

  // Trigger Backend (Pipedream API)
  const handleGenerate = async (e: React.FormEvent) => {
    e.preventDefault()
    if (credits <= 0 && !isOwner) {
      alert('Bhai Credits khatam! Apne dosto ko refer karo aur free credits kamao.')
      return
    }
    ...
    try {
      ...
      // Parse as JSON when possible, otherwise fall back to raw text.
      ...
      setAiData(normalizeResult(data))
      setHasResult(true)
      if (!isOwner) setCredits((prev) => prev - 1)
    } catch (err) {
      console.log('[v0] Generate request failed:', err)
      setErrorMessage(
          ...
          <CardContent>
            <Tabs defaultValue="login" className="w-full">
              ...
              <TabsContent value="login" className="space-y-4 pt-4">
                ...
                <Button className="w-full bg-indigo-600 hover:bg-indigo-500" onClick={() => setIsLoggedIn(true)}>
                  Sign In
                </Button>
              </TabsContent>
  ...
  // --- DASHBOARD + WRAPPED FEATURES RENDER ---
  return (
      ...
      <div className="mx-auto max-w-4xl space-y-6">
        ...
        <header className="flex flex-col items-start justify-between gap-4 rounded-xl border border-zinc-800 bg-zinc-900/60 p-4 backdrop-blur-md sm:flex-row sm:items-center">
          <div>
            ...
            <p className="text-xs text-zinc-400">{userEmail || 'user@analytics.com'}</p>
          </div>

          {/* Referral & Wallet Tracking */}
        ...
        {/* EMPTY STATE — before first successful generation */}
        {!hasResult ? (
          ...
          /* DHASU TABS DISPLAY (SUMMARY, CAPTIONS, TAGS, TITLES) */
          <Tabs defaultValue="summary" className="w-full">
            <TabsList className="grid h-12 w-full grid-cols-4 rounded-xl border border-zinc-800 bg-zinc-900 p-1 text-zinc-400">
              <TabsTrigger
                value="summary"
                className="gap-1 rounded-lg text-xs data-[state=active]:bg-zinc-800 data-[state=active]:text-white"
              >
                    ...
                    <>
                      ...
                      <div className="mt-4 flex justify-end border-t border-zinc-800/60 pt-4">
                        <Button
                          ...
                          onClick={() =>
                            handleCopy(aiData.tags.map((t) => `#${t.replace(/^#/, '')}`).join(' '), 'all-tags')
                          }
                        >
        ...
        {/* FOOTER & OWNER BYPASS KEY CONTROLLER */}
        <footer className="flex flex-col items-center justify-between gap-4 border-t border-zinc-900 pt-6 text-xs text-zinc-500 md:flex-row">
          <div className="flex w-full items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900/40 p-2 md:w-auto">
            <ShieldAlert className="h-4 w-4 shrink-0 text-zinc-400" />
            <Input
              type="password"
              placeholder="Owner Secret Key"
              className="h-7 w-32 border-zinc-700 bg-zinc-800 text-xs text-white"
              value={secretKeyInput}
              onChange={(e) => setSecretKeyInput(e.target.value)}
            />
            <Button
              size="sm"
              className="h-7 bg-zinc-800 px-2 text-xs text-zinc-300 hover:bg-zinc-700"
